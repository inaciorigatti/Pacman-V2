...
std::string Hard::format_time(unsigned int total_seconds) {
    ...
    while (window.isOpen()) {
        ...
        while (FRAME_DURATION <= lag) {
            ...
            if (dark_mode) {
                ...
                if (game_time <= 20 && 1 == game_won) { /*se o jogo foi vencido antes do tempo */
                    displayVictory();
                }
                else if(game_time > 20 && 0 == game_won) { /*se o jogo foi vencido antes do tempo */
                    displayGameOver();
                }

                // Define o tamanho do retângulo de luz
                sf::Vector2f rectSize(4 * CELL_SIZE, 4 * CELL_SIZE); // Ajuste conforme necessário
