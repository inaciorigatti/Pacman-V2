...
void GhostManager::reset(unsigned char i_level, const std::array<Position, 4>& i_ghost_positions)
{
	...
	for (Ghost& ghost : ghosts)
	{
		// Usa o fantasma azul para pegar a localização da casa e o vermelho da saida
...
{
	if (0 == i_pacman.get_energizer_timer()) //nao aumneta o tempo da wave quando o pacmna ta sobre efeito 
	{
		if (0 == wave_timer)
		{
			if (7 > current_wave)
