...
std::string Medium::format_time(unsigned int total_seconds) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::Enter) {
                if (!playerName.empty()) {
                    ...
                    ranking.addRecord(playerName, pacman.get_score(), game_time, dataAtual());
                    ranking.saveToFile();
...
void Medium::run() {
    ...
    while (window.isOpen()) {
        ...
        while (FRAME_DURATION <= lag) {
            ...
            if (dark_mode) {
                if (game_time <= 30 && 1 == game_won) { /*se o jogo foi vencido antes do tempo */
                    displayVictory();
                }
                else if (game_time > 30 && 0 == game_won) { /*se o jogo foi vencido antes do tempo */
                    displayGameOver();
                }

                // Define o tamanho do retângulo de luz
                sf::Vector2f rectSize(7 * CELL_SIZE, 7 * CELL_SIZE); // Ajuste conforme necessário
