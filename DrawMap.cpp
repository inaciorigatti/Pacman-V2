...
void draw_map(const std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, sf::RenderWindow& i_window)
{
	...
	sf::Texture texture;
	texture.loadFromFile("C:/Users/inaci/source/repos/pacmanSFML/pacmanSFML/Map" + std::to_string(CELL_SIZE) + ".png");
	...
	for (unsigned char a = 0; a < MAP_WIDTH; a++)
	{
		for (unsigned char b = 0; b < MAP_HEIGHT; b++)
		{
			...
			case Cell::Wall:
			{
				...
				//Desenhar os tuneis conforme estao conectados no jogo
				if (0 < a)
				{
					if (Cell::Wall == i_map[a - 1][b])
					{
